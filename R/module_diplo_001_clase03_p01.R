  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      observeEvent(input$render_report_button, {
        ...
        # # # Special folder
        the_package_name <- "Rscience.Diplo"
        special_folder_package <- file.path("extdata", "master_diplo", "clase03_p01")
        special_folder_local <- file.path("inst", "extdata", "master_diplo", "clase03_p01")

        # # # ---- Input objects ---- # # #
        input_old_str <- "_master"
        input_new_str <- "_mod"
        input_file_rmd   <- 'report_diplo_clase03_p01_master.Rmd'
