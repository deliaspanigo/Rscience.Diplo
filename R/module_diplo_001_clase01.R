...
module_diplo_001_clase01_server <- function(id){
  ...
  moduleServer(
    ...
    function(input, output, session) {
        ...
        if(input$"data_source" == "csv_source"){
         ...
         # Si el archivo es una base de R...
            if(input$"data_source" == "diplo_source"){
              ...
              validate(
                need(exists(input$diplo_database), 'Error 002: La base de la Diplomatura seleccionada no se encuentra en el entorno de R.\n'),
                errorClass = "ERROR"
              )
            } else
              # Si el archivo es una base de R...
      ...
      load_button_status  <- reactiveVal(FALSE)
      ...
      observeEvent(input$action_load, {
        ...
        if(load_button_counter() >= 1) runjs(sprintf('$("#%s").css({"background-color": "green",  "color": "white", "border": "none", "padding": "30px 30px", "text-align": "center", "text-decoration": "none", "display": "inline-block", "font-size": "20px", "margin": "4px 4px", "cursor": "pointer", "border-radius": "100%%"});', ns("action_load")))
        if(load_button_counter() == 0) runjs(sprintf('$("#%s").css({"background-color": "orange", "color": "white", "border": "none", "padding": "30px 30px", "text-align": "center", "text-decoration": "none", "display": "inline-block", "font-size": "20px", "margin": "4px 4px", "cursor": "pointer", "border-radius": "100%%"});', ns("action_load")))
      ...
      shiny::observeEvent(input$data_source,{
        shinyjs::reset("input-var-02-A")
        shinyjs::reset("input-var-02-B")
        #""
      ...
      control_02 <- reactive({
        ...
        req(control_01(), database(), load_button_counter()>=1)
      ...
      #--- 3.2) control_03 - Control del input$selected_var_name
      control_03 <- reactive({
      ...
      #--- 4) Creacion de selected_var_***()
      #--- 4.1) Creacion de objetos reactivos.
      ...
      #--- 4.2) control_04 - Control del selected_var_***()
      control_04 <- reactive({
      ...
      #--- 5) Control "estadistico" de la variable seleccionada
      # No puede tener celdas vacias, ni ser categorica.
      #--- 5.1) Generacion del control_05()
      control_05 <- reactive({
      ...
      #--- 5.2) Salida como texto del control_05().
      ...
      render_button_counter <- reactiveVal(0)
      ...
      observe({
        ...
        req(control_05())
        if (!is.null(control_05())) {
          if (control_05()) {
            shinyjs::enable("render_report_button")
            render_button_status(TRUE)

            # if(input$render_report_button >= 1) runjs(sprintf('$("#%s").css({"background-color": "green", "color": "white", "border": "none", "padding": "10px 20px", "text-align": "center", "text-decoration": "none", "display": "inline-block", "font-size": "16px", "margin": "4px 2px", "cursor": "pointer", "border-radius": "12px"});', ns("render_report_button")))
            if(render_button_counter() >= 1) runjs(sprintf('$("#%s").css({"background-color": "green",  "color": "white", "border": "none", "padding": "10px 20px", "text-align": "center", "text-decoration": "none", "display": "inline-block", "font-size": "16px", "margin": "4px 2px", "cursor": "pointer", "border-radius": "12px"});', ns("render_report_button")))
            if(render_button_counter() == 0) runjs(sprintf('$("#%s").css({"background-color": "orange", "color": "white", "border": "none", "padding": "10px 20px", "text-align": "center", "text-decoration": "none", "display": "inline-block", "font-size": "16px", "margin": "4px 2px", "cursor": "pointer", "border-radius": "12px"});', ns("render_report_button")))
      ...
      control_06 <- reactive({
        ...
        req(control_05(), input$render_report_button, render_button_status(), render_button_counter())
      ...
      control_07 <- reactive({
        ...
        req(control_06(), render_button_counter() > 0,
      ...
      #--- 9) Enable/Disable cada archivo
      observe({

        ns <- session$ns

        # Por defecto, todo esta Disable
        ...
        # Esta armado para todo o nada.
        # Solo si se cumple todo, se levantan todos los archivos.
        if (!is.null(control_07())) {
          if (control_07()) {
            ...
            if(input$render_report_button >= 1){
      ...
      download_counter_pdf  <- reactiveVal(0)
      download_counter_html <- reactiveVal(0)
      download_counter_word <- reactiveVal(0)
      download_counter_zip  <- reactiveVal(0)
      ...
      observe({
        ...
        ns <- session$ns
        shinyjs::runjs(sprintf('$("#%s").css("background-color", "orange");', ns("download_button_zip")))
      })
      ###---###---###---###---###---###---###---###---###---###---###---###---###---###
